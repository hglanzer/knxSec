...
\subsection{Fail-silent fault tolerance}
...
its deployment in industrial applications.
\\
\\
...
\\
\gls{mrp} does not rule out package loss in case of a failure, therefor it can be seen as improved \gls{rstp} for ring topologies, utilizing intrinsic network
redundancy.
\\
\\
In 2010, the third draft regarding IEC 62439 defined a first version of {\gls{prp}} and \gls{hsr}, booth suitable for hard real-time
...
\\
\\
\gls{prp} can tolerate a link fault without package loss by utilizing two redundant and independent networks. \glspl{danp} are connected to both networks,
upper level application data is duplicated and sent on booth lines. On the receiving side the duplicated frames are discarded by utilizing sequence numbers.
\\
\\
\gls{hsr} is based on ring-topology, but it also allows to connect different rings or even to build a ring of rings. Every node is connected
to the network by two interfaces, duplicating data from upper layers and sending one copy in clockwise, the other copy in counter-clockwise direction,
thus eliminating the need for duplicated networks but wasting about half of the available bandwidth \cite{6174793}. 
\\
In 2012, IEC 62439-3 was revised, which defined a new version of \gls{prp} and introduced \gls{hsr}. 
