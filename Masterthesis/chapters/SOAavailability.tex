...
Networks can be represented as a graph. A graph, as shown in figure \ref{fig:graph}, is an ordered pair $G=(V,E)$, with $V$ being the set of vertices and $E$
...
figure \ref{fig:stp1}: on the left side, the physical connections are shown, containing loops. 
 Therefore, \gls{stp} destroys
the loops by setting certain interfaces to blocking mode, as shown on the right side of the figure. The algorithm works as follows:
...
As it turned out the mechanisms based on \gls{stp} and it's variations were not 
