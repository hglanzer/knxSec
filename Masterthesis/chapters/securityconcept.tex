...
Every device with one connection to an unsecured \gls{knx} network two distinct connections to a secured \gls{knx} network, running the master daemon, will work
...
serving as synchronization request, containing the device's local time in seconds modulo 10, and the header flags in the secure header set accordingly, determining
...
the supplied time with it's local time modulo 10. If the timing information equals the device's own local time, the device sends a synchronization response
frame, containing it's local time and the actual counter value \ref{fig:syncResFormat}. 
\begin{figure}
  \centering
...
\end{figure}
Reducing the granularity of the sent time allows two devices to synchronize despite poor local clock synchronization.
