...
individual addresses which are responsible for delivering messages to the wanted group address. The requester can also derive a shared secret based on \gls{ecdh},
which is used to encrypt the origin \gls{knx} frame. Authenticity is guaranteed by calculating a \gls{mac2} over all frame fields except the frame check field.
\\
\\
