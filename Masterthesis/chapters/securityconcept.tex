...
\subsubsection{Global counter $Ctr_{global}$}\label{ctrInd}
The global counter is a 4 byte integer, allowing $2^{32} \approx 4,3$ billion discovery request or response messages to be sent before overflowing,
...
Every individual counter is a 4 byte integer, responsible for duplicate detection. Two distinct types of individual counters are used: outgoing, referenced
by a \gls{ga}, and incoming, also referenced by a \gls{ga}.
The main logic is shown in figure \ref{fig:dupSM}.
\\
After a new \gls{knx} frame is received on the cleartext line and the gateway knows which gateways (i.e. \gls{ia}) are responsible for the
contained \gls{ga}, the sending device determines the outgoing counter value for addressed \gls{ga}. If the counter value exists 
this means that the device already sent at least one data frame to this \gls{ga}, otherwise this is the first package frame for this \gls{ga}. In both cases,
this counter value $Ctr_{GA}$ is incremented, saved and used as outgoing $Ctr_{ind}$. After that, the cleartext frame is
encrypted and inserted into a new unicast data frame, one for each secure line.
\\
\\
Upon reception (the green transition in \ref{fig:dupSM}), at first the correctness of the \gls{mac2} is checked - if valid, the receiver checks it's incoming
individual counter values, referenced by the \gls{ga}. If the counter value for $Ctr_{ga} = 0$, this is the very first frame destined for the
contained \gls{ga}, $Ctr_{ind}$ is saved $(Ctr_{GA}$ and the inner frame is forwarded. The second frame, which will be handled eventually 
afterwards will bear the counter value $Ctr_{ind} = Ctr_{GA}$ and will therefor be discarded, eliminating the duplicate.
...
\node[state,text width=2.5cm,align=center]					at (-15,-30)			(ctrNew)		{save $Ctr_{GA/IA} = 1$}; 
\node[state,text width=2.4cm,align=center]					at (15,-30)			(ctrKnown)		{save $Ctr_{GA/IA}++$}; 
\node[state,text width=2.0cm,align=center]					at (0,-40)			(dup)		{duplicate, encrypt and send}; 
...
\path[pil,->] (rcv)  edge[] node[text width=2cm,align=left] {$Ctr_{GA/IA}$ unknown} (ctrNew); 
\path[pil,->] (rcv)  edge[] node[text width=2cm,align=left] {$Ctr_{GA/IA}$ known} (ctrKnown); 
...
\path[pil,->] (chk)  edge[]   node[text width=1cm] {$Ctr < Ctr_{old}$} (discard); 
\path[pil,->] (chk)  edge[]   node[text width=1cm] {$Ctr \ge Ctr_{old}$} (fwd); 
