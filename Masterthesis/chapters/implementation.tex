...
\end{lstlisting}
globalMutex is a global variable of type \textit{pthread\_mutex\_t}, shared between all threads which are accessing the shared resource. After calling pthread\_mutex\_lock(),
...
shortcut one secure communication line such that the discovery message will not reach it's recipient(s). Receiving the discovery message on the other line and injecting the frame to the
