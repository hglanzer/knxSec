...
first, and then process them in parallel\footnote{Counter Mode, see Section \ref{confidentiality}}. 
\\
...
\gls{ccm} \cite{ccm} combines \gls{cbc} for authentication with \gls{ctr} for encryption for 128 bit blocks.
...
% Valid values are $2 \leq L \leq 18$.
...
% Thus, authenticated encryption is achieved in a manner also called 'mac-then-encrypt'. 
% \\
...
\gls{dh} and therefore also the \gls{ecdlp} can be generalized to $n$ parties, obtaining one key in common. The key negotiation procedure for three parties, using
classical \gls{dh}, is
shown in Figures \ref{fig:dh1} and \ref{fig:dh2}, where it is assumed that all three parties $A$, $B$ and $C$ already agreed upon $(p,g)$.
\begin{figure}
    \centering
...
\subsubsection{Signatures based on RSA}
...
\end{align*}
$x'$ is converted to an integer an reduced $\pmod n$ to obtain $r'$. If $r = r'$, the signature is accepted, proving the authenticity of the message.
