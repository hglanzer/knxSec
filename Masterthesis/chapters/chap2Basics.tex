...
transmitted physically from one user to another.
A classification of threats against information security is given in Figure \ref{fig:attacks}.  
\begin{figure}[h]
    \centering
...
A passive attack, as shown in Figure \ref{fig:passiveAttack},
 is undetectable and aimed against confidentiality. Here, the attacker $E$ \textbf{intercepts} the data transmitted between the two entities.
 After that, the attacker could \textbf{release} the data itself to an outside entity, or \textbf{analyze} the data, allowing him to get knowledge of some kind of meta data
 like origin, destination and frequency of a data flow.
Passive attacks are often used as starting point for obtaining knowledge which can later to be used to mount 
additional attacks - to exclude passive attacks, encryption is sufficient. 
\begin{figure}[h]
\centering
 \begin{tikzpicture}[scale=0.2]
\tikzstyle{every node}+=[inner sep=2pt]
