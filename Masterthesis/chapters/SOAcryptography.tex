...
\\
\\
A finite field can be constructed by taking the set of integers $\mathbb{Z}\pmod p$, with $p \in \mathbb{P}$, thus restricting the set of all integers to the set
$\mathbb{Z}_p = \{0, 1, ..., p-1\}$.
...
RSA, published in 1977 by Ron Rivest, Adi Shamir, and Leonard Adleman \cite{RSA}, relies on the hardness of finding the prime factors of a big composite number.
...
For key generation, 
two large primes $p, q$, which should be of about same size, are chosen randomly, with $N = pq$. Additionally, a public exponent $e$ and a private exponent $d$ are chosen s.t.
\begin{align}\label{ed}
 e * d \equiv 1 \pmod {\varphi(N)}
\end{align}
$\varphi(N)$, Euler's totient function, counts the number of integers relatively prime to $N$. For a prime $p$, $\varphi(p) = (p-1)$, therefore for the product
