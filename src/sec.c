...
void saveGlobalCounter(void *env, uint8_t *buffer)
{
	...
	for(i = GLOBALCOUNTSIZE; i>0;i=i-1)
	{
		...
		thisEnv->secGlobalCount[GLOBALCOUNTSIZE-i-1] = buffer[i-1];
		exp = exp * 256;
	...
	*/
	switch(type)
	{
		...
		case syncRes:
			...
			secBufferMAC[thisEnv->id][7] = thisEnv->secGlobalCount[3];	// global Counter
			secBufferMAC[thisEnv->id][8] = thisEnv->secGlobalCount[2];	// ...
			secBufferMAC[thisEnv->id][9] = thisEnv->secGlobalCount[1];	// ...
			secBufferMAC[thisEnv->id][10] = thisEnv->secGlobalCount[0];	// global counter
